...
import com.diazbumma.binarytree.BinaryTree;
import com.diazbumma.binarytree.NodeTree;
import com.diazbumma.linkedlist.Iterator;
import com.diazbumma.linkedlist.LinkedList;
import com.diazbumma.linkedlist.NodeList;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //String stringData = "0 5 7 2 5 3 7 9 3 2 5 5 8 1";
        ...
        for (String s : data) {
            ...
            cities.addInOrder(new NodeList(s));
        }

        Iterator cityIterator = cities.getIterator();
        printLinkedListData(cityIterator);
        System.out.println("========");

        //printing second data
        Iterator secondIterator = cities.getIterator();
        secondIterator.next();
        System.out.println(secondIterator.next());

        //remove third data
        Iterator thirdIterator = cities.getIterator();
        thirdIterator.next();
        thirdIterator.next();
        thirdIterator.remove();
        thirdIterator.previous();
        thirdIterator.previous();
        printLinkedListData(thirdIterator);

        //BinaryTree Section
        String stringData2 = "Jakarta Yogyakarta Magelang Semarang Surabaya Serang Bandung Denpasar";

        BinaryTree cities2 = new BinaryTree();

        String[] data2 = stringData2.split(" ");
        for (String s : data2) {
            // create new item with value set to the string s
            cities2.insert(new NodeTree(s));
        }

        System.out.println("Print City");
        cities2.printPreorder();
        cities2.printInorder();
        cities2.printPostorder();
        cities2.printLevelOrder();
    }

    public static void printLinkedListData(Iterator iterator) {
        while (true) {
            if (iterator.hasNext()) {
